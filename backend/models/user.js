...
async function createUser(userData) {
  try {
    ...
    return { id: result.insertedId, username: newUser.username, email: newUser.email };
  } catch (error) {
    throw error;
  }
}
...
async function findByEmail(email) {
  try {
    ...
    return user ? { id: user._id, username: user.username, email: user.email, password: user.password } : null;
  } catch (error) {
    throw error;
  }
}
...
module.exports = { setUserCollection, createUser, findByEmail, findById };
